...
AUDIT_API_KEY=<the flags key from AUDIT_API_KEYS, role write>
```
...
| Action | Request |
|---|---|
| `flags.flag.create` | `POST /v1/flags` |
| `flags.flag.update` | `PATCH /v1/flags/:key` |
| `flags.flag.delete` | `DELETE /v1/flags/:key` |
| `flags.env.update` | `PATCH /v1/flags/:key/envs/:env` |
| `flags.env.copy` | `POST /v1/flags/:key/envs/:env/copy` |
...
`meta` carries the request patch or a summary (counts) where useful; secrets never appear in it. Query the audit service by `source=flags`, `actionPrefix=flags.` or by target to reconstruct what happened to an entity.
