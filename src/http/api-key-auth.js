...
/**
 * Bearer API-key authentication with read/write roles and optional environment scoping. Every
...
export class ApiKeyAuth {
  ...
  static require(need) {
    ...
    return async (request) => {
      ...
      if (role !== need && role !== 'readwrite') throw new FlagError('FORBIDDEN', `this API key has no ${need} access`);
    };
  }
  ...
  static assertEnv(key, env) {
    if (key.envs && !key.envs.includes(env)) throw new FlagError('FORBIDDEN', `this API key has no access to environment "${env}"`);
  }
